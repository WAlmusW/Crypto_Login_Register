...
class FirestoreClient:
    def check_registration_status(device_udid):
    ...
    def register_user(username, password, device_udid):
        try:
            # Reference to the "users" collection in Firestore
            users_ref = db.collection('users')
            ...
            try:
                ...
                latest_user_key = 1  # Default to 1 if no user exists yet

            # Create a new user with an incremental key
            user_key = f'user{latest_user_key}'
            users_ref.document(user_key).set({
                'id': latest_user_key,
